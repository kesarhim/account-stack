...
export class TableComponentComponent implements OnInit {
  ...
  public pendingBalance: Array<string> = new Array<string>(...[Balance.PENDING, Balance.COMPLETED]);
  ...
  if (this.tableConfig.displayedColumns.length > 0) {
    this.tableConfig.displayedColumns.forEach(coloums => {
      if (coloums.applyFilter) {
        let filter: ITableFilter = { filterName: coloums.header, dataSource: Array<string>(), fieldName: coloums.name }
        if(filter.fieldName === 'balanceAmount'){
          filter.dataSource = this.pendingBalance;
        }else{
          filter.dataSource = this.getFilterDataSource(filter.fieldName,coloums.showUpperCase || false) ?? new Array<string>();
        }
...
getFilterDataSource = (fieldName: string,showUpperCase:boolean): Array<string> => {
